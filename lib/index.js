 ...
 */
const nameInterpolator = (input, regExp) => ({ file, content, index }) => {
...
class FastCSSSplitWebpackPlugin {
  ...
  constructor ({
    ...
    this.options = {
      ...
      filename: nameInterpolator(filename, regExp),
      preserve,
      regExp,	
      defer,
      chunks
  ...
  chunksMapping (compilation, chunks, done) {
    ...
    const isInChunks = (chunk) => !Array.isArray(this.options.chunks) ||  this.options.chunks.indexOf(chunk.name) !== -1;
    chunks.filter(isInChunks).map((chunk) => {
